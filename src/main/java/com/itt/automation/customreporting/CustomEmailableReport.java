...
public class CustomEmailableReport implements IReporter {
	...
	public void generateReport(List<XmlSuite> xml, List<ISuite> suites, String outdir) {
		...
		LOG.info("STARTED CREATING EMAILABLE REPORT NOW");
		try {
		...
		try {
			LOG.info("START HTML METHOD STARTED");
			startHtml(m_out);
			LOG.info("GENERATE SUITE SUMMARY REPORT METHOD STARTED");
			generateSuiteSummaryReportExecutionTime(suites);
			generateSuiteSummaryReport(suites);
			LOG.info("GENERATE METHOD SUMMARY REPORT METHOD STARTED");
			generateMethodSummaryReport(suites);
			LOG.info("GENERATE METHOD DETAIL REPORT METHOD STARTED");
			generateMethodDetailReport(suites);
			LOG.info("END HTML METHOD STARTED");
			endHtml(m_out);
			LOG.info("WRITING HTML CONTENT INTO EMAILABLE REPORT NOW");
			m_out.flush();
	...
	private void resultSummary(ISuite suite, IResultMap tests, String testname,
		...
		if (tests.getAllResults().size() > 0) {
			...
			for (ITestNGMethod method : getMethodSet(tests, suite)) {
				...
				if (mq == 0) {
					String id = (m_testIndex == null
							? null
							: "t" + Integer.toString(m_testIndex));
				...
				buff.append("<td style=\"text-align:center\"><a href=\"#m" + m_methodIndex + "\">"
						...
						+ (null == testInstanceName
								? ""
								: "<br>(" + testInstanceName + ")")
						+ "</td>" + "<td style=\"text-align:center\" class=\"numi\">" + resultSet.size()
						+ "</td>" + "<td style=\"text-align:center\">" + start + "</td>"
						+ "<td style=\"text-align:center\" class=\"numi\">" + (end - start) + "</td>"
						+ "</tr>");
				break;
			}
			if (mq > 0) {
	...
	private void generateForResult(ITestResult ans, ITestNGMethod method,
		...
		if (hasParameters) {
			...
			for (Object p : parameters) {
				m_out.println(
						"<td style=\"text-align:center\">" + Utils.escapeHtml(Utils.toString(p)) + "</td>");
			}
			m_out.println("</tr>");
		}
		...
		if (hasReporterOutput || hasThrowable) {
			...
			if (hasThrowable) {
				boolean wantsMinimalOutput = ans
						.getStatus() == ITestResult.SUCCESS;
				if (hasReporterOutput) {
	...
	private Collection<ITestNGMethod> getMethodSet(IResultMap tests,
		...
		for (IInvokedMethod im : invokedMethods) {
			if (tests.getAllMethods().contains(im.getTestMethod())
					&& this.isMethodNotExpectedSkipped(im.getTestResult())
					&& this.isMethodNotExpectedFailure(im.getTestResult())) {
				r.add(im);
			}
		}
	...
	public boolean isMethodNotExpectedSkipped(ITestResult testResult) {
		boolean isMethodNotExpectedSkippedFlag = false;
		isMethodNotExpectedSkippedFlag = !((testResult
				.getAttribute("expectedSkippedValue") != null)
				&& (testResult.getAttribute("expectedSkippedValue").toString()
						.equalsIgnoreCase("true")));
		return isMethodNotExpectedSkippedFlag;
	}

	public boolean isMethodNotExpectedFailure(ITestResult testResult) {
		boolean isMethodNotExpectedFailureFlag = false;
		isMethodNotExpectedFailureFlag = !((testResult
				.getAttribute("retryFailedTestValue") != null)
				&& (testResult.getAttribute("retryFailedTestValue").toString()
						.equalsIgnoreCase("true")));
		return isMethodNotExpectedFailureFlag;
	}

	public void generateSuiteSummaryReportExecutionTime(List<ISuite> suites) {
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult r : tests.values()) {
				...
				if (overview != null
					...
					AppUrl = overview.getAttribute("APP_URL").toString();
				}
				if (overview != null
						&& overview.getAttribute("TEST_EXECUTION_ENVIRONMENT") != null) {
					TestEnvironment = overview.getAttribute("TEST_EXECUTION_ENVIRONMENT").toString();
				}
				if (overview != null
						&& overview.getAttribute("BROWSER_NAME") != null) {
					BrowserName = overview.getAttribute("BROWSER_NAME").toString();
				}
				
				if (overview != null
						&& overview.getAttribute("BROWSER_VERSION") != null) {
			...
			m_out.print("<b>OVERALL SUMMARY:</b>");
...
//				summaryCell(new String[]{BrowserVersion});
	...
	private void summaryCell(String v, boolean isgood) {
		m_out.print("<td style=\"text-align:center\" class=\"numi" + (isgood ? "" : "_attn") + "\">" + v
				+ "</td>");
	}
	...
	private void startSummaryRow(String label) {
		...
		m_out.print("<tr" + (m_row % 2 == 0 ? " class=\"stripe\"" : "")
				+ "><td style=\\\"text-align:center\\\"><a href=\"#t"
